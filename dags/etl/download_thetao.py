...
def download_thetao():
    output_dir = "/opt/airflow/nc/thetao"

    # ❗ ไม่พยายามสร้างโฟลเดอร์ — ถ้าไม่มี ถือว่า host setup ไม่พร้อม
    if not os.path.exists(output_dir):
        raise FileNotFoundError(
            f"❌ Directory not found: {output_dir}\n"
            f"💡 Please create it on the host machine under ./airflow/nc/thetao before running."
        )
    ...
    for province, bounds in provinces.items():
        output_file = os.path.join(output_dir, f"{province}_thetao.nc")
        ...
        print(f"✅ Downloaded thetao: {output_file}")
