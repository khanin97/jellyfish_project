...
def transform(ti):
    ...
    df = df.dropna(subset=["thetao", "time"])
    ti.xcom_push(key=XCOM_KEY, value=df.reset_index(drop=True).to_json())

def clean(ti):
    df = pd.read_json(io.StringIO(ti.xcom_pull(key=XCOM_KEY)))
    df_daily = df.groupby([df["time"].dt.date, "province"])[thetao].mean().reset_index()
    df_daily.rename(columns={"time": "date", "thetao": "thetao_mean"}, inplace=True)
    ti.xcom_push(key=XCOM_KEY, value=df_daily.reset_index(drop=True).to_json())

def save_to_db(ti):
    df = pd.read_json(io.StringIO(ti.xcom_pull(key=XCOM_KEY)))
    con = duckdb.connect(DB_PATH)
    con.execute("CREATE OR REPLACE TABLE province_daily_thetao AS SELECT * FROM df")
    print("✅ Overwritten: province_daily_thetao in DuckDB")
